...
class Mata_pelajaran_nilai_model extends CI_Model
{
	...
	function get_data($param = array())
	{
		...
		$this->db->select("
			...
			d.keterangan
		");
		$this->db->order_by('d.nilai_id, c.nis, b.nama');
		$this->db->from('mata_pelajaran_nilai_detail a');
		$this->db->join('user b', 'b.user_id = a.user_id');
		$this->db->join('user_siswa c', 'c.user_id = b.user_id');
		$this->db->join('mata_pelajaran_nilai d', 'd.nilai_id = a.nilai_id');
		$get = $this->db->get();
		return $get;
	}
	...
	function get_data_siswa($param = array())
	{
		...
		if(!empty($param))
		{
			...
			{
				$query_str = ", x.nilai";
				$this->db->join("mata_pelajaran_nilai_detail x", "a.user_id = x.user_id AND x.nilai_id = '$param[id]'");
			}
			else
			{
				if(!empty($param['kelas']))
		...
		$this->db->select('a.user_id, a.nama, b.nis' . $query_str);
		$this->db->from('user a');
