...
class Api_guru extends CI_Controller
{
	...
	function update_fcm()
	{
		...
		$data_user = $this->login_model->get_data_guru_device($data_post['device_id']);
		if(!empty($data_user))
		{
			$param  = array('fcm' => $data_post['token']);
			$proses = $this->login_model->update_login($param, $data_user->user_id);
			if($proses)
			{
				$respon = array(
					'status' 	=> '200', 
