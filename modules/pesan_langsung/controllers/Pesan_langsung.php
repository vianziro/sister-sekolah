...
class Pesan_langsung extends CI_Controller
{
	...
	public function index($id = '')
	{
		...
		{
			unset($param['opt_jenis_user']['wali_kelas']);
			unset($param['opt_jenis_user']['operator']);
			...
			foreach($data_post['penerima'] as $key => $c)
			{
				...
				if($this->pesan_kotak_model->insert($param_pesan, $this->login_uid, $c, $target))
				{
					...
					if(!empty($fcm))
					{
						...
						{
							$this->fcm->insertNotifikasiWali($c, 'Pesan Baru', $isi_notifikasi, $fcm);						
						}
						else
						{
							$this->fcm->insertNotifikasiUser($c, 'Pesan Baru', $isi_notifikasi, $fcm);
						}						
					}
				}
			}
